...
def score_kicker(stats: dict) -> tuple[float, dict[str, int | float]]:
    """
    Score a kicker.
...
def score_head_coach(game_info: dict) -> tuple[float, dict[str, int | float]]:
    """
    Score a head coach.
...
def score_offensive_line(
    team_stats: dict, ol_touchdowns: int = 0
) -> tuple[float, dict[str, int | float]]:
    """
