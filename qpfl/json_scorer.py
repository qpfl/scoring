...
from .scorer import QPFLScorer


def load_rosters(rosters_path: str | Path) -> dict[str, list[dict]]:
...
def load_lineup(lineup_path: str | Path, week: int) -> dict[str, dict]:
    ...
    if data.get('week') != week:
        print(f"Warning: Lineup file week ({data.get('week')}) doesn't match expected week ({week})")
...
def score_week_from_json(
    ...
    # Build fantasy teams
    teams = []
    for team_abbrev in rosters.keys():
...
def save_week_scores(
    ...
    sorted_teams = sorted(teams_data, key=lambda t: t['total_score'], reverse=True)
...
def update_standings_json(
    ...
    """
    from .schedule import parse_schedule_file
    ...
    for week_path in week_data_paths:
        ...
        # Top half scoring
        teams_by_score = sorted(week_data.get('teams', []), key=lambda x: x.get('total_score', 0), reverse=True)
        ...
        while i < len(teams_by_score):
            ...
            tied_teams = []
            while i < len(teams_by_score) and teams_by_score[i].get('total_score', 0) == current_score:
