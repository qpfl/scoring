...
def update_team_name_file(team: str, new_name: str, week: int, github_token: str) -> tuple[bool, str]:
    ...
    content["team_names"][team] = [
        entry for entry in content["team_names"][team]
        if entry.get("effective_week", 1) < week
    ]

    content["team_names"][team].append({
        "name": new_name,
        "effective_week": week
    })
...
class handler(BaseHTTPRequestHandler):  # noqa: N801
    ...
    def do_GET(self):
        ...
        self.send_header("Access-Control-Allow-Origin", "*")
        self.end_headers()
        self.wfile.write(json.dumps({"status": "Team Name API is running", "method": "GET"}).encode())
