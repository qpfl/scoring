...
def github_api_request(
    path: str, method: str = 'GET', data: dict = None, max_retries: int = 3
) -> tuple[bool, dict | str]:
    ...
    try:
        if method == 'GET':
            ...
            for attempt in range(max_retries):
                ...
                if (
                    'transaction_log' in path
                    and current_content
                    and 'transactions' in data['content']
                ):
                    ...
                    merged_txns = existing_txns + [
                        t for t in new_txns if t.get('timestamp') not in existing_timestamps
                    ]
...
def handle_taxi_activation(data: dict) -> tuple[int, dict]:
    ...
    if taxi_player['position'] != roster_player['position']:
        return 400, {
            'error': f'Position mismatch: {taxi_player["position"]} vs {roster_player["position"]}'
        }
    ...
    success, msg = github_api_request(
        'data/rosters.json',
        'PUT',
        {
...
def handle_fa_activation(data: dict) -> tuple[int, dict]:
    ...
    fa_player = next(
        (p for p in fa_pool['players'] if p['name'] == player_to_add and p.get('available', True)),
        None,
    )
    if not fa_player:
    ...
    if fa_player['position'] != roster_player['position']:
        return 400, {
            'error': f'Position mismatch: {fa_player["position"]} vs {roster_player["position"]}'
        }
    ...
    success, msg = github_api_request(
        'data/fa_pool.json',
        'PUT',
        {'message': f'FA pool update: {player_to_add} activated by {team}', 'content': fa_pool},
    )
    if not success:
        return 500, {'error': msg}
...
def handle_propose_trade(data: dict) -> tuple[int, dict]:
    ...
    success, msg = github_api_request(
        ...
        'PUT',
        {'message': f'Trade proposed: {team} to {trade_partner}', 'content': pending},
    )
...
def execute_trade(trade: dict) -> tuple[bool, str, dict]:
    ...
    success, msg = github_api_request(
        'data/rosters.json',
        'PUT',
        {'message': f'Trade executed: {proposer} <-> {partner}', 'content': rosters},
    )
...
def handle_respond_trade(data: dict) -> tuple[int, dict]:
    ...
    success, msg = github_api_request(
        'data/pending_trades.json',
        'PUT',
        {'message': f'Trade {trade_id} {trade["status"]}', 'content': pending},
    )
...
def handle_cancel_trade(data: dict) -> tuple[int, dict]:
    ...
    success, msg = github_api_request(
        'data/pending_trades.json',
        'PUT',
        {'message': f'Trade {trade_id} cancelled by {team}', 'content': pending},
    )
...
def handle_save_tradeblock(data: dict) -> tuple[int, dict]:
    ...
    success, msg = github_api_request(
        'data/trade_blocks.json',
        'PUT',
        {'message': f'Trade block updated: {team}', 'content': trade_blocks},
    )
