...
def archive_season(
    season_dir: Path, web_dir: Path, prev_season: int, dry_run: bool = False
) -> None:
    ...
    if not historical_path.exists():
        ...
        if standings_path.exists() and weeks_dir.exists():
            ...
            for week_file in sorted(
                weeks_dir.glob('week_*.json'), key=lambda x: int(x.stem.split('_')[1])
            ):
...
def create_new_season_dir(
    season_dir: Path, new_season: int, teams: list[dict], dry_run: bool = False
) -> None:
...
def get_teams_from_data(data_dir: Path) -> list[dict]:
    ...
    return [
        ...
        },
        {
            'abbrev': 'SLS',
            'name': 'TBD',
            'owner': 'Stephen Schmidt',
            'owner_key': 'stephen_schmidt',
        },
...
def main():
    ...
    update_file_pattern(
        export_path, r'season:\s*int\s*=\s*\d{4}', f'season: int = {new_season}', dry_run
    )
    update_file_pattern(
        export_path,
