...
def print_discrepancies(discrepancies: list[dict], total_checked: int, verbose: bool = True):
    ...
    if mismatches:
        ...
        for d in mismatches:
            ...
            print(
                f'  {d["position"]:<5} {player_str:<30} {d["excel_score"]:>7.1f} {d["calculated_score"]:>7.1f} {d["difference"]:>+7.1f}'
            )
    ...
    if not_found:
        ...
        for d in not_found:
            player_str = f'{d["player"]} ({d["nfl_team"]})'[:30]
            print(
                f'  {d["position"]:<5} {player_str:<30} {d["excel_score"]:>7.1f} (bye/injured/no stats)'
            )
        print()
