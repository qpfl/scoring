...
def looks_like_team_name(part: str) -> bool:
    ...
    if re.search(r'\b(taxi|pick)\b', part, re.IGNORECASE):
        return False
    if re.search(
        r'\b(?:RB|WR|TE|QB|K|D/ST|DST)\b|202[0-9]|\b(?:round|1st|2nd|3rd|4th)\b|[()]',
        part,
        re.IGNORECASE,
    ):
        return False
...
def parse_old_trade_message(message: str):
    ...
    def explicit_team_name(part):
        if not re.match(r'^to\s+.+', part, re.IGNORECASE) and not re.search(
            r'\s+gets?:?$', part, re.IGNORECASE
        ):
            return None
    ...
    for part in parts:
        ...
        if in_corresponding_moves:
            continue
        if (
            not has_explicit_headers
            and (current_team is None or looks_like_team_name(part))
            and (current_team is None or len(teams) < 2)
        ):
